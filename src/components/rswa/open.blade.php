<div>
    ...
    <form action="{{route('fmt.rswa.store')}}" method="post" class="fmt_box" enctype="multipart/form-data">
        ...
        <div class="fmt_headline">Add a "Read the Sentence ; record the Audio"</div>
        <div>
...
{{--  --}}
<form action="{{route('fmt.rswa.csv')}}" method="POST" enctype='multipart/form-data' style="margin:20px 40px;">
    @csrf
    <input type="integer" name="problem_set_id" value="{{$pbs72 ?? ''}}" hidden style="border:1px solid #000000;">
    <input type="integer" name="format_type_id" value="{{$fmt72 ?? ''}}" hidden style="border:1px solid #000000;">
